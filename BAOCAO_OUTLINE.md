# KHUNG MỤC LỤC CHI TIẾT BÁO CÁO ĐỒ ÁN TỐT NGHIỆP (BAOCAO_OUTLINE.md)

Tài liệu này xác lập cấu trúc mục lục chi tiết đến cấp 3 cho toàn bộ Báo cáo Đồ án Tốt nghiệp Kỹ sư AI tại Trường Đại học Công nghệ Thông tin và Truyền thông Việt - Hàn (VKU) năm 2026. Cấu trúc tuân thủ nghiêm ngặt **Hướng dẫn Báo cáo ĐATN Định hướng NCKH năm 2026 v2.docx** (gồm 5 chương) và tích hợp các chuẩn mực khoa học từ các tài liệu tham khảo thực tế của hệ thống Dịch thuật Giọng nói song hướng Anh - Việt thời gian thực (S2ST).

---

## MỤC LỤC HỌC THUẬT BÁO CÁO

### TRANG THỦ TỤC & DẪN NHẬP
*   **TRANG TIÊU ĐỀ (Title Page)**
*   **TRANG NỘI DUNG (Content Page)**
*   **LỜI CẢM ƠN (Acknowlegements)**
...
*   **4.4. Kết quả thực nghiệm hệ thống máy khách Client App (PySide6)** `[⚠️ Bug Đang Fix]`
    *   4.4.1. Kết quả kiểm thử giao thức kết nối WebSocket và hiển thị đồ họa văn bản dịch interim/final `[⚠️ Bug Đang Fix]`
    *   4.4.2. Kết quả kiểm tra tính ổn định, giảm hiện tượng giật cục của luồng phát âm thanh AdaptiveJitterPlayer `[⚠️ Bug Đang Fix]`
    *   4.4.3. Đánh giá tính cô lập âm thanh loopback chống vọng tiếng khi họp trực tuyến song hướng thực tế `[⚠️ Bug Đang Fix]`
*   **4.5. Kết quả thực nghiệm và huấn luyện mô hình Piper TTS** `[🔄 Đang Train - Dữ liệu đang cập nhật]`
    *   4.5.1. Phân tích đường cong hội tụ Mel loss và KL loss qua 3000 epochs trên Google Colab GPU T4 `[🔄 Đang Train - Dữ liệu đang cập nhật]`
    *   4.5.2. Biểu đồ hiệu năng tiêu thụ phần cứng (VRAM & GPU Utilization) ghi nhận tự động qua gpu_log.csv `[🔄 Đang Train - Dữ liệu đang cập nhật]`
    *   4.5.3. Kết quả lượng tử hóa FP32 sang FP16 (.onnx): Dung lượng mô hình, tài nguyên CPU/GPU và so sánh phổ âm (Spectrogram) `[🔄 Đang Train - Dữ liệu đang cập nhật]`
    *   4.5.4. Đối sánh điểm số chất lượng giọng nói tự nhiên khách quan (NISQA MOS) và chủ quan (Human MOS) `[🔄 Đang Train - Dữ liệu đang cập nhật]`
...
*   **DANH MỤC TÀI LIỆU THAM KHẢO (Reference List)** (Định dạng chuẩn APA 7th Edition)
*   **PHỤ LỤC (Appendices)** (Mã nguồn core_ai, kịch bản lọc dataset hybrid, file cấu hình warmup)
*   **TÓM TẮT BÁO CÁO DẠNG BÀI BÁO KHOA HỌC THEO MẪU IEEE (IEEE Paper Summary)** (Tối đa 10 trang)
